...
function GetAllActiveCities() {
  ...
  return (
    <div className="p-6">
      <h2 className="text-3xl font-bold mb-6 text-gray-800">Explore Cities</h2>

      {loading ? (
        <p className="text-gray-500">Loading cities...</p>
      ) : cities.length === 0 ? (
        <p className="text-gray-500">No active cities found.</p>
      ) : (
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        {selectedCity.images?.map((img, index) => (
          <img
            ...
            src={img}
            alt="city"
            className="rounded-lg object-cover h-36 w-full hover:scale-105 transition duration-300"
          />
        ))}
      </div>
