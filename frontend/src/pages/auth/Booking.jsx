...
const services = [
  {
    id: 1,
    name: "Classic Consultation",
    duration: "30 min",
    price: 499,
    icon: "✦",
    desc: "Perfect for first-timers seeking expert advice",
    tag: null,
  },
  {
    id: 2,
    ...
    price: 999,
    icon: "◈",
    desc: "Deep-dive personalized experience",
    tag: "POPULAR",
  },
  {
    id: 3,
    ...
    price: 1799,
    icon: "❋",
    desc: "Full luxury treatment with priority access",
    tag: "BEST VALUE",
  },
];

const COUPONS = { FIRST50: 50, SAVE20: 20, VIP10: 10 };

const timeSlots = [
  { t: "09:00 AM", avail: "open" },
  { t: "10:00 AM", avail: "filling" },
  { t: "11:00 AM", avail: "full" },
  { t: "12:00 PM", avail: "open" },
  { t: "02:00 PM", avail: "filling" },
  { t: "03:00 PM", avail: "open" },
  { t: "04:00 PM", avail: "full" },
  { t: "05:00 PM", avail: "open" },
  { t: "06:00 PM", avail: "filling" },
];
...
export default function BookingFlow() {
  ...
  const dayObj = selected.day !== null ? days[selected.day] : null;
  const discount = appliedCoupon
    ? Math.round(((svc?.price || 0) * COUPONS[appliedCoupon]) / 100)
    : 0;
  const finalPrice = svc ? svc.price - discount : 0;

  const canNext1 = selected.service !== null;
  const canNext2 = selected.day !== null && selected.time !== null;
  const canNext5 =
    payMethod === "upi"
      ? upiId.includes("@")
  ...
  const handleSendOtp = () => {
    ...
    setOtpTimer(30);
    setOtpError("");
    setTimeout(() => setOtp(["4", "2", "0", "1", "3", "7"]), 1500);
  };
  ...
  const handleNext3 = () => {
    ...
    setFormErrs({});
    setLoading(true);
    setLoadingMsg("Saving your details...");
    setTimeout(() => {
      setLoading(false);
      setStep(4);
      handleSendOtp();
  ...
  const handleApplyCoupon = () => {
    ...
    if (COUPONS[c]) {
      setAppliedCoupon(c);
      setCouponMsg({ ok: true, text: `${COUPONS[c]}% discount applied!` });
    } else {
  ...
  const handlePay = () => {
    ...
    setTimeout(() => {
      setLoadingMsg("Confirming booking...");
      setTimeout(() => {
        setLoading(false);
        setBooked(true);
      }, 1000);
  ...
  const reset = () => {
    setStep(1);
    setSelected({ service: null, day: null, time: null });
  ...
  const styles = `
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    .svc-card:hover{border-color:#d4a882;background:#fdf9f6;}
    .svc-card.sel{border-color:#c8855a;background:#fdf5ef;box-shadow:0 0 0 3px rgba(200,133,90,.1);}
    .svc-tag{
      position:absolute;top:-1px;right:14px;
      background:#c8855a;color:#fff;
      font-size:8px;font-weight:700;letter-spacing:.1em;
      padding:3px 8px;border-radius:0 0 6px 6px;
    }
    .svc-icon{font-size:18px;color:#c8855a;width:28px;text-align:center;flex-shrink:0;}
    .svc-info{flex:1;}
    .svc-name{font-size:15px;font-weight:600;color:#1a1714;}
    .svc-desc{font-size:12px;color:#9a9490;margin-top:2px;}
    .svc-meta{text-align:right;flex-shrink:0;}
    .svc-price{font-size:17px;color:#c8855a;font-weight:700;}
    .svc-dur{font-size:11px;color:#b0aaa4;margin-top:2px;}

    /* Calendar */
    .days-scroll{display:flex;gap:8px;overflow-x:auto;padding-bottom:4px;}
    .days-scroll::-webkit-scrollbar{height:3px;}
    .days-scroll::-webkit-scrollbar-thumb{background:#e0dbd4;border-radius:2px;}
    .day-btn{
      flex-shrink:0;width:54px;padding:10px 0;
      border:1.5px solid #ede9e3;border-radius:10px;
      background:#fff;color:#6b6560;
      cursor:pointer;text-align:center;transition:all .2s;
    }
    .day-btn.weekend{background:#faf9f7;}
    .day-btn:hover{border-color:#d4a882;background:#fdf9f6;}
    .day-btn.sel{border-color:#c8855a;background:#fdf5ef;box-shadow:0 0 0 3px rgba(200,133,90,.1);}
    .day-name{font-size:9px;text-transform:uppercase;letter-spacing:.08em;font-weight:600;color:#b0aaa4;}
    .day-num{font-family:'Instrument Serif',serif;font-size:22px;line-height:1.1;margin:3px 0;color:#1a1714;}
    .day-month{font-size:9px;color:#c8855a;font-weight:500;}
    .day-btn.sel .day-name{color:#c8855a;}
    .day-btn.sel .day-num{color:#c8855a;}

    .time-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:20px;}
    .tslot{display:flex;flex-direction:column;align-items:center;gap:3px;}
    .time-btn{
      width:100%;padding:10px;
      border:1.5px solid #ede9e3;border-radius:8px;
      background:#fff;color:#6b6560;
      cursor:pointer;font-family:'Plus Jakarta Sans',sans-serif;font-size:12px;font-weight:500;
      transition:all .2s;
    }
    .time-btn:hover:not(.full){border-color:#d4a882;background:#fdf9f6;color:#1a1714;}
    .time-btn.sel{border-color:#c8855a;background:#fdf5ef;color:#c8855a;font-weight:600;box-shadow:0 0 0 3px rgba(200,133,90,.1);}
    .time-btn.full{opacity:.4;cursor:not-allowed;text-decoration:line-through;background:#f8f7f5;}
    .avail-tag{font-size:9px;font-weight:600;letter-spacing:.04em;}
    .avail-tag.filling{color:#d97706;}
    .avail-tag.full{color:#ef4444;}
    .avail-tag.open{color:#16a34a;}
    ...
    .form-input{
      ...
      border:1.5px solid #ede9e3;border-radius:10px;
      padding:11px 14px;color:#1a1714;
    ...
    .otp-input{
      ...
      background:#faf9f7;border:1.5px solid #ede9e3;border-radius:10px;
      color:#1a1714;font-family:'Instrument Serif',serif;font-size:26px;
      outline:none;transition:all .2s;
    }
    ...
    .otp-verified{
      ...
      border-radius:10px;color:#15803d;font-size:12px;font-weight:600;letter-spacing:.06em;
    }
    .resend-btn{
      background:none;border:none;color:#c8855a;
      cursor:pointer;font-family:'Plus Jakarta Sans',sans-serif;
      font-size:12px;font-weight:600;text-decoration:underline;margin-top:10px;
    }
    ...
    .coupon-input{
      flex:1;background:#faf9f7;border:1.5px solid #ede9e3;border-radius:10px;
      padding:10px 14px;color:#1a1714;
      font-family:'Plus Jakarta Sans',sans-serif;font-size:12px;font-weight:600;
      outline:none;text-transform:uppercase;transition:all .2s;
    }
    ...
    .coupon-btn{
      background:#c8855a;border:none;color:#fff;
      cursor:pointer;padding:10px 18px;
      font-family:'Plus Jakarta Sans',sans-serif;font-size:11px;font-weight:700;
      border-radius:10px;letter-spacing:.06em;text-transform:uppercase;transition:all .2s;
    }
    ...
    .price-box{
      background:#faf9f7;border:1.5px solid #ede9e3;border-radius:12px;
    ...
    .btn-primary{
      flex:1;padding:14px;
      background:#c8855a;border:none;border-radius:10px;
      color:#fff;font-family:'Plus Jakarta Sans',sans-serif;
      font-size:13px;font-weight:700;letter-spacing:.04em;
      cursor:pointer;transition:all .2s;
      box-shadow:0 2px 12px rgba(200,133,90,.25);
    }
    ...
    .btn-ghost{
      padding:14px 20px;
      background:#fff;border:1.5px solid #ede9e3;border-radius:10px;
      color:#6b6560;font-family:'Plus Jakarta Sans',sans-serif;
    ...
    .spinner{
      ...
      border:2px solid #ede9e3;border-top-color:#c8855a;
    ...
    .success-icon{
      ...
      background:#fdf5ef;border:2px solid #c8855a;
      display:flex;align-items:center;justify-content:center;
      margin:0 auto 24px;font-size:28px;color:#c8855a;
    ...
    .new-btn{
      ...
      border:1.5px solid #ede9e3;color:#6b6560;
    ...
    .bank-btn{
      padding:10px;border:1.5px solid #ede9e3;border-radius:8px;
            ...
            <div className="body">
              <div className="success-wrap">
                ...
                <div className="success-details">
                  <div>
                    {svc?.name} · {selected.time}
                  </div>
                  <div>
                    {dayObj?.label}, {dayObj?.date} {dayObj?.month}
                  </div>
                  <div className="success-paid">
                    ₹{finalPrice.toLocaleString("en-IN")} paid via{" "}
                    {payMethod === "upi"
                      ? "UPI"
                      : payMethod === "card"
                        ? "Card"
          ...
          <div className="body">
              ...
              <>
                <div className="svc-grid">
                  {services.map((s) => (
                    <div
                      key={s.id}
                      className={`svc-card ${selected.service === s.id ? "sel" : ""}`}
                      onClick={() =>
              ...
              <>
                ...
                <div className="days-scroll">
                  {days.map((d, i) => (
                    <button
                      key={i}
                      className={`day-btn ${selected.day === i ? "sel" : ""} ${d.isWeekend ? "weekend" : ""}`}
                      onClick={() => setSelected((p) => ({ ...p, day: i }))}
                    >
                ...
                <div className="time-grid">
                  {timeSlots.map(({ t, avail }) => (
                    <div key={t} className="tslot">
                      <button
                        className={`time-btn ${selected.time === t ? "sel" : ""} ${avail === "full" ? "full" : ""}`}
                        disabled={avail === "full"}
                        onClick={() => setSelected((p) => ({ ...p, time: t }))}
                      >
                        {t}
                      </button>
                      <div className={`avail-tag ${avail}`}>
                        {avail === "full"
                          ? "Fully Booked"
                          : avail === "filling"
                            ? "Filling fast"
            ...
            {step === 3 && (
              <>
                ...
                <div className="btn-row">
                  <button className="btn-primary" onClick={handleNext3}>
                    Send OTP & Continue →
                  </button>
                </div>
              </>
              ...
              <>
                <div style={{ textAlign: "center", marginBottom: 4 }}>
                  ...
                  <div className="hint" style={{ textAlign: "center" }}>
                    Demo: code is auto-filled — just tap Verify
                  </div>
                </div>
                {otpVerified ? (
            ...
            {step === 5 && (
              <>
                <div className="price-box">
                  <div className="prow">
                    <span className="pkey">Service Price</span>
                    <span className="pval">
                  ...
                  {appliedCoupon && (
                    <div className="prow">
                      <span className="pkey">Discount ({appliedCoupon})</span>
                ...
                <div className="coupon-row">
                  <input
                    ...
                    placeholder="Enter code (try FIRST50)"
                    value={coupon}
                  ...
                  {appliedCoupon && (
                    <div className="srow">
                      ...
                      <div className="sval" style={{ color: "#16a34a" }}>
                        −₹{discount.toLocaleString("en-IN")} ({appliedCoupon})
                      </div>
                    </div>
                  )}
                  <div className="srow">
