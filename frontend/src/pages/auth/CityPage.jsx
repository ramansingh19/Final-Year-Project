...
function CityDetails() {
    ...
    <div className="absolute inset-0 flex flex-col items-center justify-end p-8 lg:p-12 text-center text-white max-w-3xl mx-auto">
      <div className="inline-flex items-center px-4 py-2 rounded-full bg-white/20 backdrop-blur-xl border border-white/40 mb-4">
        <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m6 0h6M4 12h16M5 7h14v10H5V7z" />
        </svg>
  ...
  <div className="max-w-5xl mx-auto px-6 lg:px-8 -mt-10 relative z-10">
    ...
    <section className="mb-20">
      <div className="grid md:grid-cols-3 gap-6">
        <div className="group bg-white/80 backdrop-blur-xl rounded-2xl p-8 shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 border border-white/70 hover:border-emerald-200/50">
          ...
          <h3 className="text-xl font-bold text-gray-900 mb-3 group-hover:text-emerald-600">Famous For</h3>
        ...
        <div className="group bg-white/80 backdrop-blur-xl rounded-2xl p-8 shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 border border-white/70 hover:border-amber-200/50">
          ...
          <h3 className="text-xl font-bold text-gray-900 mb-3 group-hover:text-amber-600">Best Time</h3>
          <p className="text-gray-600 leading-relaxed">{city.bestTimeToVisit}</p>
        </div>
        ...
        <div className="group bg-white/80 backdrop-blur-xl rounded-2xl p-8 shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 border border-white/70 hover:border-indigo-200/50">
          ...
          <h3 className="text-xl font-bold text-gray-900 mb-3 group-hover:text-indigo-600">Daily Budget</h3>
          <p className="text-2xl font-bold text-gray-900">₹{city.avgDailyBudget}</p>
          <p className="text-sm text-gray-500 mt-1 font-medium uppercase tracking-wide">Per Person</p>
        </div>
      </div>
    </section>
    ...
    <section>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {[
          { icon: '🏨', title: 'Hotels', desc: 'Best stays & deals', path: `/hotels?city=${city.name}` },
          { icon: '📍', title: 'Places', desc: 'Top attractions', path: `/places?city=${city.name}` },
          { icon: '🍽', title: 'Restaurants', desc: 'Local flavors', path: `/restaurants?city=${city.name}` }
        ].map((item) => (
          <div
            key={item.title}
